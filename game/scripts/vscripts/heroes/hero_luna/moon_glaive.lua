...
-- the results are stored in particle_names and projectile_speeds so it doesn't have to reload the KV file each time
function findProjectileInfo(class_name)
	return particle_names["base"], projectile_speeds["base"]
end
...
function moon_glaive_dummy_created( keys )
	...
	if caster:GetOwner():GetClassname() == "player" then
		local unit_ability = caster:FindAbilityByName("flamethrower")
		...
		ability.dmgBonus = unit_ability:GetLevelSpecialValueFor("damage_bonus", unit_ability:GetLevel() - 1)
   ...
   local lastSource = ability.projectileFrom
   local lastTarget = ability.projectileTo
   ...
   for k,v in pairs(unitsNearTarget) do
	...
	if ability.projectileTo == ability.projectileFrom then
		killDummy(caster, target)
		return
	end
